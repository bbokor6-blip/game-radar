...
function GameSpotlight({game,rank}){
  ...
  return <article className="wrGame">
    <div className="wrGameHead">
      ...
      <span className="wrRadarIndex" title="RadarIndex">{game.interest?.score??"—"}</span>
    </div>
    <p>{why(game)}</p>
...
export default function Weekly(){
  ...
  const games=(data.games||[]).slice().sort((a,b)=>(b.interest?.score||0)-(a.interest?.score||0));
  ...
  return <main className="wrPage">
    <header className="wrHeader">
      ...
      <details className="wrMenu"><summary>☰</summary><div><a href="/scores">GameRadar</a><a href="/bets">BetRadar</a><a href="/weekly">Weekly Radar</a><a href="/">Home</a></div></details>
    </header>

    <section className="wrHero">
      <div><span className="wrEyebrow">{league==="nfl"?"NFL WEEKLY RADAR":"COLLEGE FOOTBALL WEEKLY RADAR"}</span><h1>{league==="nfl"?"The week, without the noise.":"The games that matter next."}</h1><p>{league==="nfl"?"The best games, strongest RadarIndex scores, and betting looks to know before kickoff.":"A fast look at the ranked clashes, competitive spots, and BetRadar picks worth knowing before Saturday."}</p></div>
    ...
    {error?<div className="grEmpty">{error}</div>:loading?<div className="grEmpty">Building the weekly radar…</div>:<>
      <section className="wrSection"><div className="wrSectionHead"><div><span>START HERE</span><h2>5 games to know</h2></div><a href={"/scores?league="+league+"&week="+weekOffset}>Full board →</a></div><div className="wrGames">{top.map((g,i)=><GameSpotlight key={g.id} game={g} rank={i+1}/>)}</div></section>
      ...
      <section className="wrSection wrTakeaway"><span>THE READ</span><h2>{top[0]?((top[0].away?.location||top[0].away?.short)+" @ "+(top[0].home?.location||top[0].home?.short)):"This week's board"} leads the Radar.</h2><p>{top[0]?why(top[0]):"Check back as the slate fills in."}</p></section>
    </>}
  </main>;
}
