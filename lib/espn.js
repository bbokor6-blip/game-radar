...
export async function fetchConsensusOdds(sport, eventId, competitionId=eventId) {
  ...
  return (payload.items || []).map((item) => {
    ...
    return {
      ...
      homeMargin,
      overUnder: Number.isFinite(overUnder) ? overUnder : null,
      homeSpreadOdds: price(item?.homeTeamOdds?.spreadOdds),
      awaySpreadOdds: price(item?.awayTeamOdds?.spreadOdds),
      overOdds: price(item?.overOdds),
      underOdds: price(item?.underOdds)
    };
