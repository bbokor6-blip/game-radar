...
import { gameIndexTier } from "./indexTiers";
import { calibrateSlateGameIndexes } from "./gameIndexCalibration";
...
export function previewGame(game) {
  ...
  const context=gameMetadata(game);
  const h = recordPct(game.home.record), a = recordPct(game.away.record);
  const gap = Math.abs(h-a);

  if (spread != null) {
    if (spread <= 2.5) { score += 35; reasons.push("market expects a toss-up"); }
    else if (spread <= 4.5) { score += 29; reasons.push("very tight betting line"); }
    else if (spread <= 7.5) { score += 23; reasons.push("one-score projection"); }
    else if (spread < 10) { score += 16; reasons.push("projected under-10 margin"); }
    else if (spread <= 13.5) { score += 6; reasons.push("within striking range"); }
  ...
  if (gap <= .15) score += 5;
  if(context.bothWinning){score+=4;reasons.push("both teams have winning records");}
  if(context.undefeatedInvolved){score+=3;reasons.push("undefeated team involved");}
  if(context.conferenceGame){score+=5;reasons.push("conference game");}
  if(context.rivalry){score+=8;reasons.push("rivalry");}
  if(context.postseason){score+=6;reasons.push("postseason stakes");}
  if(context.kickoffWindow==="PRIMETIME"){
    score+=8;
    reasons.push("primetime spotlight");
  }

  if (game.sport === "cfb") {
    if (rankedMatchup(game)) { score += 20; reasons.push("Top 25 showdown"); }
    else if (ranked(game)) { score += 9; reasons.push("Top 25 team"); }
    const ranks=[game.home.rank,game.away.rank].filter(Boolean);
    if (ranks.length && Math.min(...ranks)<=10) { score += 6; reasons.push("top-10 team"); }
    // Surface less obvious games that the market expects to be competitive.
    // This is intentionally independent from BetIndex: it finds good viewing,
    // not an edge against the line.
    if(!ranked(game)&&spread!=null&&spread<=4.5&&(context.bothWinning||context.bothPower)){
      score+=8;
      reasons.push("under-the-radar toss-up");
    }
  } else {
    const quality=(h+a)/2;
    if (quality >= .66) { score += 8; reasons.push("strong teams"); }
  }
  ...
  const tier=gameIndexTier(score).label;
...
export function recapGame(game) {
  ...
  score=clamp(Math.round(score));
  return {score,tier:gameIndexTier(score).label,reason:reasons.slice(0,3).join(" · ")||"Final score recap"};
}
...
export function scoreGame(game) {
  ...
  if (game.sport === "cfb") {
    if (rankedMatchup(game)) { score += 10; reasons.push("ranked matchup"); }
    else if (ranked(game)) { score += 5; reasons.push("ranked team involved"); }
    if (upset(game)) { score += 17; reasons.push("ranked upset brewing"); }
  }
  if(context.rivalry)score+=2;
  if(context.postseason)score+=4;
...
export function rankGames(games) {
  ...
  // GameIndex is a discovery ranking, so calibrate it relative to the slate.
  // Four rank buckets make each tier useful even when an absolute formula would
  // bunch an early-season slate in the middle. Small slates keep raw scores so
  // a single quiet game cannot be labeled Must Watch by default.
  return calibrateSlateGameIndexes(rankedGames,score=>gameIndexTier(score).label);
}
