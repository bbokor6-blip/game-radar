...
export function previewGame(game) {
  ...
  if(context.conferenceGame){score+=4;reasons.push("conference game");}
  if(context.rivalry){score+=4;reasons.push("rivalry");}
  if(context.postseason){score+=6;reasons.push("postseason stakes");}
  if(context.kickoffWindow==="PRIMETIME"){score+=2;}

  if (game.sport === "cfb") {
    if (rankedMatchup(game)) { score += 18; reasons.push("Top 25 showdown"); }
    else if (ranked(game)) { score += 8; reasons.push("Top 25 team"); }
