...
export function previewGame(game) {
  ...
  if (spread != null) {
    if (spread <= 2.5) { score += 38; reasons.push("market expects a toss-up"); }
    else if (spread <= 4.5) { score += 31; reasons.push("tight betting line"); }
    else if (spread <= 7.5) { score += 21; reasons.push("competitive spread"); }
    else if (spread <= 10.5) { score += 10; reasons.push("within striking range"); }
    else if (spread >= 17) { score -= 13; reasons.push("market expects a mismatch"); }
  } else {
