...
function sideOpportunity(game,profiles,market,vegasHistory,sport,projection){
  ...
  const trendUsable=atsUsable(home)&&atsUsable(away);
  const homeScore=.65*home.atsPct+.35*home.recentAtsPct;
  const awayScore=.65*away.atsPct+.35*away.recentAtsPct;
  const atsDelta=homeScore-awayScore;
  const modelEdge=Number.isFinite(projection?.homeMargin)?projection.homeMargin-market.homeMargin:0;
  const modelSignal=clamp(modelEdge/10,-1,1);
  const trendSignal=trendUsable?clamp(atsDelta/.25,-1,1):0;
  const combinedSignal=trendUsable?(modelSignal*.70)+(trendSignal*.30):modelSignal;

  let side=null;
  if(combinedSignal>.03)side='home';
  else if(combinedSignal<-.03)side='away';
  else{
  ...
  const sampleScore=trendUsable?clamp(sample/5,0,1):0;
  const agreement=market.providerCount>=2?clamp(1-(market.spreadDispersion||0)/2.5,0,1):0;
  const modelStrength=Math.min(18,Math.abs(modelEdge)*2.4);
  const trendStrength=trendUsable?Math.min(8,Math.abs(atsDelta)*24):0;
  let index=46+modelStrength+trendStrength+(sampleScore*7)+(agreement*5);
  if(market.providerCount>=3)index+=4;
  if(trendUsable&&Math.sign(modelSignal)&&Math.sign(trendSignal)&&Math.sign(modelSignal)!==Math.sign(trendSignal))index-=5;
  if(!trendUsable)index-=4;
  if(sport==="cfb"&&trendUsable&&sample<3)index-=4;
  if(Math.abs(market.homeMargin)>14)index-=Math.min(6,(Math.abs(market.homeMargin)-14)*.5);
  ...
  if(trendUsable){
    evidence.push(chosenTeam.short+' covered '+chosen.atsWins+' of '+chosen.atsGames+' verified lines');
    evidence.push(otherTeam.short+' covered '+other.atsWins+' of '+other.atsGames+' verified lines');
  }else{
    evidence.push('ATS coverage incomplete: '+chosenTeam.short+' '+chosen.atsLineGames+'/'+chosen.games+', '+otherTeam.short+' '+other.atsLineGames+'/'+other.games);
  }
  if(vegas?.text)evidence.push(vegas.text);
  if(Number.isFinite(projection?.homeMargin))evidence.push('Projected margin '+(projection.homeMargin>0?game.home.short:game.away.short)+' by '+Math.abs(projection.homeMargin).toFixed(1));
  ...
  return {
    ...
    index,
    label:index>=80?'BEST BET':index>=70?'STRONG':index>=60?'LEAN':'PASS',
    why:spreadReason(chosenTeam,otherTeam,chosen,other,pick,vegas,trendUsable,projection,game),
    evidence,
    vegasContext:vegas?.text||null,
    highConviction:index>=80,
    noBrainer:index>=85,
    evidenceQuality:{sample,providerCount:market.providerCount||0,trendUsable,atsCoverage:{chosen:chosen.atsCoverage,other:other.atsCoverage}},
    side
  };
}
...
function totalOpportunity(game,profiles,market,vegasHistory,sport,projection){
  ...
  const home=profiles.get(key(game.home))||empty;
  const away=profiles.get(key(game.away))||empty;
  const trendUsable=totalsUsable(home)&&totalsUsable(away);
  ...
  const distance=trendUsable?Math.abs(combined-.5):0;

  const modelEdge=Number.isFinite(projection?.total)?projection.total-market.total:0;
  const combinedSignal=trendUsable?(clamp(modelEdge/14,-1,1)*.70)+(clamp((combined-.5)/.2,-1,1)*.30):clamp(modelEdge/14,-1,1);
  let over=null;
  if(combinedSignal>.03)over=true;
  else if(combinedSignal<-.03)over=false;
  else{
  ...
  let index=42+Math.min(13,Math.abs(modelEdge)*1.5)+(distance*30)+(sampleScore*6)+(agreement*4);
  if(!trendUsable)index-=4;
  if(market.providerCount>=3)index+=4;
  const vegas=totalVegasContext(market,over,vegasHistory,sport);
  if(vegas?.support===1)index+=5;
  if(vegas?.support===-1)index-=4;
  index=Math.round(clamp(index-6,40,69));

  const americanOdds=over?market.overOdds:market.underOdds;
  const evidence=[];
  if(trendUsable){
    evidence.push(over?game.home.short+' went over '+home.overWins+' of '+home.overGames:game.home.short+' stayed under '+Math.max(0,home.overGames-home.overWins)+' of '+home.overGames);
    evidence.push(over?game.away.short+' went over '+away.overWins+' of '+away.overGames:game.away.short+' stayed under '+Math.max(0,away.overGames-away.overWins)+' of '+away.overGames);
  }else evidence.push('Totals history incomplete: '+game.home.short+' '+home.totalLineGames+'/'+home.games+', '+game.away.short+' '+away.totalLineGames+'/'+away.games);
  if(vegas?.text)evidence.push(vegas.text);
  ...
  return {
    ...
    payout:payoutForOdds(americanOdds,10),
    index,
    label:index>=60?'LEAN':'PASS',
    why:trendUsable?totalReason(game,home,away,market,over):'Complete scores are included, but verified totals are available for only '+home.totalLineGames+' of '+home.games+' '+game.home.short+' games and '+away.totalLineGames+' of '+away.games+' '+game.away.short+' games. This is a model lean, not a full-season totals trend.',
    evidence,
    vegasContext:vegas?.text||null,
    highConviction:false,
    noBrainer:false,
    evidenceQuality:{sample,providerCount:market.providerCount||0,trendUsable,totalCoverage:{home:home.totalCoverage,away:away.totalCoverage}},
