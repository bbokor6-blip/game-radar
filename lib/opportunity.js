...
function sideOpportunity(game,profiles,market,vegasHistory,sport,projection){
  ...
  const trendSignal=clamp(atsDelta/.25,-1,1);
  const combinedSignal=(modelSignal*.70)+(trendSignal*.30);
  ...
  const trendStrength=Math.min(8,Math.abs(atsDelta)*24);
  let index=46+modelStrength+trendStrength+(sampleScore*7)+(agreement*5);
  if(market.providerCount>=3)index+=4;
  if(Math.sign(modelSignal)&&Math.sign(trendSignal)&&Math.sign(modelSignal)!==Math.sign(trendSignal))index-=5;
  if(sport==="cfb"&&sample<3)index-=4;
  ...
  return {
    ...
    index,
    label:index>=80?'HIGH CONFIDENCE':index>=70?'MODEL LEAN':index>=60?'WATCH':'FULL-SLATE PICK',
    why:spreadReason(chosenTeam,otherTeam,chosen,other,pick,vegas),
    evidence,
    vegasContext:vegas?.text||null,
    highConviction:index>=80,
    noBrainer:index>=85,
    evidenceQuality:{sample,providerCount:market.providerCount||0},
    side
  };
}
...
function totalOpportunity(game,profiles,market,vegasHistory,sport,projection){
  ...
  const combinedSignal=(clamp(modelEdge/14,-1,1)*.70)+(clamp((combined-.5)/.2,-1,1)*.30);
  let over=null;
  ...
  return {
    ...
    payout:payoutForOdds(americanOdds,10),
    index,
    label:index>=60?'TOTALS WATCH':'FULL-SLATE PICK',
