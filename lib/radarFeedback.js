...
export function buildFeedbackProfile(ledger,{includeExcluded=false,retrospectiveWeight=0}={}){
  const options={includeExcluded,retrospectiveWeight};
  ...
  for(const key of ["<60","60-69","70-79","80+"])byBand[key]=summarize(graded.filter(p=>band(p.betRadarIndex)===key),options);
  ...
  return {overall:summarize(graded,options),byType,byBand,byLeague,byLeagueType,retrospectiveWeight};
}
