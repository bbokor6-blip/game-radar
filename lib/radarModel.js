...
export function buildPowerModel(games,sport){
  ...
  for(const [id,t] of teams){
    ...
    // Keep opponent-adjusted team strength intact; express small-sample uncertainty
    // in the confidence layer instead of shrinking every team back toward average.
    const power=.78*srs+.22*recentMargin;
    const offense=t.games?leagueAvg+(t.pf/t.games-leagueAvg)*sampleWeight:leagueAvg;
    const defenseAllowed=t.games?leagueAvg+(t.pa/t.games-leagueAvg)*sampleWeight:leagueAvg;
    output.set(id,{...t,power,offense,defenseAllowed,sampleWeight});
  }
