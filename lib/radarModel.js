...
function blankTeam(team){
  return {id:teamKey(team),short:team?.short||'',games:0,pf:0,pa:0,entries:[]};
}
...
export function buildPowerModel(games,sport){
  ...
  for(const game of completed){
    ...
    home.entries.push({date:game.date,margin:hs-as-homeField,opp:away.id});
    away.entries.push({date:game.date,margin:as-hs+homeField,opp:home.id});
    pointSum+=hs+as;appearances+=2;
  }

  const leagueAvg=appearances?pointSum/appearances:(sport==='nfl'?22.5:28);
  let ratings=new Map();
  for(const [id,t] of teams)ratings.set(id,t.entries.length?mean(t.entries.map(e=>e.margin)):0);

  for(let iteration=0;iteration<8;iteration++){
    const next=new Map();
    for(const [id,t] of teams){
      const value=t.entries.length?mean(t.entries.map(e=>e.margin+(ratings.get(e.opp)||0))):0;
      next.set(id,value);
    }
  ...
  for(const [id,t] of teams){
    ...
    const recentMargin=recent.length?mean(recent.map(e=>e.margin)):0;
    const shrink=clamp(t.games/4,0,1);
    const srs=ratings.get(id)||0;
    const power=(.78*srs+.22*recentMargin)*shrink;
    const offense=t.games?leagueAvg+(t.pf/t.games-leagueAvg)*shrink:leagueAvg;
    const defenseAllowed=t.games?leagueAvg+(t.pa/t.games-leagueAvg)*shrink:leagueAvg;
    output.set(id,{...t,power,offense,defenseAllowed,shrink});
  }
...
export function projectGame(game,model){
  ...
  const powerMargin=hPower-aPower+model.homeField;
  const homePts=model.leagueAvg+.55*(hOff-model.leagueAvg)+.45*(aDef-model.leagueAvg)+model.homeField/2;
  const awayPts=model.leagueAvg+.55*(aOff-model.leagueAvg)+.45*(hDef-model.leagueAvg)-model.homeField/2;
...
function confidenceIndex({edge,p,reliability,dispersion,providerCount,edgeScale,calibration}){
...
export function evaluateGame(game,projection,market,calibration){
  const reliability=clamp(Math.min(projection.homeGames,projection.awayGames)/4,.45,1);
  ...
  if(Number.isFinite(market.homeMargin)){
    ...
    const p=normalCdf(absEdge/calibration.marginSigma);
    ...
    const confidence=confidenceIndex({
      ...
      providerCount:market.providerCount||1,edgeScale:5,calibration
    });
    spreadBet={
      type:'SPREAD',
